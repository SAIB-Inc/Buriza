...
public class VaultEncryptionTests
{
    ...
    [Fact]
    public void Decrypt_WithWeakMemoryCost_ThrowsCryptographicException()
    {
        // Arrange - Create vault then tamper with KDF params
        EncryptedVault vault = VaultEncryption.Encrypt(TestWalletId, ToBytes(TestMnemonic), TestPassword);

        // Tamper with KDF to use weak memory (below OWASP minimum of 19 MiB)
        EncryptedVault weakVault = vault with
        {
            Kdf = new Argon2Params { Memory = 1024, Iterations = 3, Parallelism = 4 }  // 1 MiB - too weak
        };

        // Act & Assert - Should reject weak parameters
        CryptographicException ex = Assert.Throws<CryptographicException>(() =>
            VaultEncryption.Decrypt(weakVault, TestPassword));
        Assert.Contains("memory cost", ex.Message, StringComparison.OrdinalIgnoreCase);
    }

    [Fact]
    public void Decrypt_WithWeakIterations_ThrowsCryptographicException()
    {
        // Arrange
        EncryptedVault vault = VaultEncryption.Encrypt(TestWalletId, ToBytes(TestMnemonic), TestPassword);

        // Tamper with KDF to use weak iterations (below minimum of 2)
        EncryptedVault weakVault = vault with
        {
            Kdf = new Argon2Params { Memory = 65536, Iterations = 1, Parallelism = 4 }
        };
    ...
    [Fact]
    public void Decrypt_WithInvalidParallelism_ThrowsCryptographicException()
    {
        // Arrange
        EncryptedVault vault = VaultEncryption.Encrypt(TestWalletId, ToBytes(TestMnemonic), TestPassword);

        // Tamper with KDF to use invalid parallelism (0 is invalid)
        EncryptedVault weakVault = vault with
        {
            Kdf = new Argon2Params { Memory = 65536, Iterations = 3, Parallelism = 0 }
        };
    ...
    [Fact]
    public void Decrypt_WithExcessiveParallelism_ThrowsCryptographicException()
    {
        // Arrange
        EncryptedVault vault = VaultEncryption.Encrypt(TestWalletId, ToBytes(TestMnemonic), TestPassword);

        // Tamper with KDF to use excessive parallelism (above max of 16)
        EncryptedVault weakVault = vault with
        {
            Kdf = new Argon2Params { Memory = 65536, Iterations = 3, Parallelism = 100 }
        };
    ...
    [Fact]
    public void Decrypt_WithMinimumValidParams_Succeeds()
    {
        // Arrange - Create with OWASP minimum parameters
        KeyDerivationOptions minOptions = KeyDerivationOptions.Minimum;
        EncryptedVault vault = VaultEncryption.Encrypt(TestWalletId, ToBytes(TestMnemonic), TestPassword, options: minOptions);
    ...
    [Fact]
    public void Decrypt_WithProvidedOptions_BypassesValidation()
    {
        // Arrange - Create vault with default params
        EncryptedVault vault = VaultEncryption.Encrypt(TestWalletId, ToBytes(TestMnemonic), TestPassword);

        // Act - Provide explicit options (caller takes responsibility)
        // This should work because we're passing the same options used to encrypt
        byte[] decryptedBytes = VaultEncryption.Decrypt(vault, TestPassword, KeyDerivationOptions.Default);
