...
public static class VaultEncryption
{
    ...
    public static SecretVerifierPayload CreateSecretVerifier(string secret, KeyDerivationOptions? options = null)
    {
        ...
        try
        {
            return new SecretVerifierPayload
            {
                Version = 1,
                Salt = Convert.ToBase64String(salt),
    ...
    public static bool VerifySecret(string secret, SecretVerifierPayload verifier, KeyDerivationOptions? options = null)
    {
        ...
        options ??= new KeyDerivationOptions
        {
            ...
            MemoryCost = verifier.MemoryCost,
            Parallelism = verifier.Parallelism,
            KeyLength = verifier.KeyLength,
            SaltSize = KeyDerivationOptions.Default.SaltSize,
            IvSize = KeyDerivationOptions.Default.IvSize,
            TagSize = KeyDerivationOptions.Default.TagSize
        };

        if (options.KeyLength <= 0)
            throw new CryptographicException("Invalid verifier: key length is not valid");
