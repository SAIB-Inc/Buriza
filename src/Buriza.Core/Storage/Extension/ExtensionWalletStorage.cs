...
public class ExtensionWalletStorage(IJSRuntime jsRuntime) : IWalletStorage
{
    ...
    public async Task SaveDataServiceApiKeyAsync(DataServiceType serviceType, string apiKey, string password, CancellationToken ct = default)
    {
        EncryptedVault vault = VaultEncryption.Encrypt(0, apiKey, password);
        string json = JsonSerializer.Serialize(vault);
        await _js.InvokeVoidAsync("buriza.storage.set", ct, GetDataServiceApiKeyVaultKey(serviceType), json);
    ...
    {
        string? json = await _js.InvokeAsync<string?>("buriza.storage.get", ct, GetDataServiceApiKeyVaultKey(serviceType));
        if (string.IsNullOrEmpty(json))
            return null;
    ...
    {
        await _js.InvokeVoidAsync("buriza.storage.remove", ct, GetDataServiceApiKeyVaultKey(serviceType));
    ...
    {
        string? json = await _js.InvokeAsync<string?>("buriza.storage.get", ct, DataServiceConfigsKey);
        if (string.IsNullOrEmpty(json))
            return [];
    ...
    {
        string json = JsonSerializer.Serialize(configs);
        await _js.InvokeVoidAsync("buriza.storage.set", ct, DataServiceConfigsKey, json);
    }
